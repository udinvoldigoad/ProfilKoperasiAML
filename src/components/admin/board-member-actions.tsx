...
export function BoardMemberActions({ memberId, name }: { memberId: string; name: string }) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  async function handleDelete() {
    if (!window.confirm(`Hapus pengurus "${name}"? Tindakan ini tidak dapat dibatalkan.`)) return;
    setError(null);
    setLoading(true);
    try {
      const response = await fetch(`/api/admin/struktur/${memberId}`, { method: "DELETE" });
      const data = await response.json();
      if (!response.ok) {
        setError(data.error ?? "Gagal menghapus pengurus.");
        return;
      }
      router.refresh();
    } catch {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <Link
        href={`/admin/struktur/${memberId}/edit`}
