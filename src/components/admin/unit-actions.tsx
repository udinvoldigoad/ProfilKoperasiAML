...
export function UnitActions({ unitId, name }: { unitId: string; name: string }) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  async function handleDelete() {
    if (!window.confirm(`Hapus unit "${name}"? Tindakan ini tidak dapat dibatalkan.`)) return;
    setError(null);
    setLoading(true);
    try {
      const response = await fetch(`/api/admin/unit/${unitId}`, { method: "DELETE" });
      const data = await response.json();
      if (!response.ok) {
        setError(data.error ?? "Gagal menghapus unit.");
        return;
      }
      router.refresh();
    } catch {
